 ...
 *
 * https://github.com/ninjin/barnes-hut-sne/blob/master/quadtree.cpp
 *
...
QuadTree::QuadTree(particle_t *inp_data, int N)
{
    ...
    for(int n = 0; n < N; n++) {
        for(int d = 0; d < QT_NO_DIMS; d++) {
			//mean_Y[0] = x-value, mean_Y[1] = y-value...
            /*mean_Y[d] += inp_data[n * QT_NO_DIMS + d].x;
            if(inp_data[n * QT_NO_DIMS + d].x < min_Y[d]) min_Y[d] = inp_data[n * QT_NO_DIMS + d].x;
            if(inp_data[n * QT_NO_DIMS + d].x > max_Y[d]) max_Y[d] = inp_data[n * QT_NO_DIMS + d].x;
			*/
			mean_Y[0] += inp_data[n].x;
			mean_Y[1] += inp_data[n].y;
			if(inp_data[n].x < min_Y[0]) min_Y[0] = inp_data[n].x;
			if(inp_data[n].x < min_Y[1]) min_Y[1] = inp_data[n].y;
			if(inp_data[n].x > max_Y[0]) max_Y[0] = inp_data[n].x;
			if(inp_data[n].y > max_Y[1]) max_Y[1] = inp_data[n].y;
...
QuadTree::QuadTree(particle_t* inp_data, int N, double inp_x, double inp_y, double inp_hw, double inp_hh)
{
...
// Constructor for quadtree with particular size and parent -- build the tree, too!
QuadTree::QuadTree(QuadTree* inp_parent, particle_t* inp_data, int N, double inp_x, double inp_y, double inp_hw, double inp_hh)
{
...
QuadTree::QuadTree(particle_t* inp_data, double inp_x, double inp_y, double inp_hw, double inp_hh)
{
    init(NULL, inp_data, inp_x, inp_y, inp_hw, inp_hh);
}
...
QuadTree::QuadTree(QuadTree* inp_parent, particle_t* inp_data, double inp_x, double inp_y, double inp_hw, double inp_hh)
{
    init(inp_parent, inp_data, inp_x, inp_y, inp_hw, inp_hh);
}
...
void QuadTree::init(QuadTree* inp_parent, particle_t* inp_data, double inp_x, double inp_y, double inp_hw, double inp_hh)
{
    parent = inp_parent;
    data = inp_data;
    is_leaf = true;
...
void QuadTree::setData(particle_t* inp_data)
{
    data = inp_data;
}
...
bool QuadTree::insert(int new_index)
{
	...
	double point[] = {data[new_index].x, data[new_index].y};
    ...
    for(int n = 0; n < size; n++) {
		...
		for(int d = 0; d<1; d++){
			...
			if(point[0] != data[index[n]].x) {duplicate = false; break;}
			if(point[1] != data[index[n]].y) {duplicate = false; break;}
        }
...
bool QuadTree::isCorrect()
{
    for(int n = 0; n < size; n++) {
		...
		double point[] = {data[index[n]].x, data[index[n]].y};
...
void QuadTree::computeNonEdgeForces(int point_index, double theta, double neg_f[], double* sum_Q)
{
	...
	*/
	buff[0] = data[point_index].x;
	buff[1] = data[point_index].y;
	buff[0] -= center_of_mass[0];//[point_index][0];
	buff[1] -= center_of_mass[1];//[point_index][1];
	D += buff[0] * buff[0];
	D += buff[1] * buff[1];
    ...
    for(int n = 0; n < N; n++) {
        ...
        for(int i = row_P[n]; i < row_P[n + 1]; i++) {
			...
			*/
			buff[0] = data[n].x;
			buff[1] = data[n].y;
			buff[0] -= data[col_P[i]].x;
			buff[1] -= data[col_P[i]].y;
			D += buff[0] * buff[0];
			D += buff[1] * buff[1];
...
void QuadTree::print() 
{
    ...
    if(is_leaf) {
        ...
        for(int i = 0; i < size; i++) {
			...
			double point[] = {data[index[i]].x, data[index[i]].y};
